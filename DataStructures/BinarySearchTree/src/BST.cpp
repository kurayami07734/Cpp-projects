...
void BinarySearchTree::display_BFS() {
    ...
    while (!q.empty()) {
        std::cout << q.front() << " ";
        q.pop();
        if (ptr->left != nullptr) q.push(ptr->left->data);
        if (ptr->right != nullptr) q.push(ptr->right->data);
        ptr = find(q.front());
    }
    std::cout << "\n";
}
...
void traverse_pre_order(BSTNode *node) {
    std::cout << node->data << " ";
    if (node->left != nullptr) traverse_pre_order(node->left);
    if (node->right != nullptr) traverse_pre_order(node->right);
}

void traverse_post_order(BSTNode *node) {
    if (node->left != nullptr) traverse_post_order(node->left);
    if (node->right != nullptr) traverse_post_order(node->right);
    std::cout << node->data << " ";
}
...
void traverse_in_order(BSTNode *node) {
    if (node->left != nullptr) traverse_in_order(node->left);
    std::cout << node->data << " ";
    if (node->right != nullptr) traverse_in_order(node->right);
}
...
BSTNode *BinarySearchTree::remove_node(int key) { BSTNode *node = find(key); }
